...
| F13 | **Push Notifications**                | Web Push API notifications for likes, comments, and new followers. Delivered via VAPID-signed Web Push through a `NotificationPort` adapter; users opt in via an in-app prompt and the service worker renders notifications. Future: company-wide featured videos / broadcast. |
...
| F13 — Push Notifications                                                | 🟡 Medium | Delivered      | 🟢 Medium      | ⭐⭐⭐     | Web Push (VAPID) wired through `NotificationPort` for like/comment/follow events; opt-in prompt + service worker installed. Broadcast/featured-video notifications still pending.                                                                          |
