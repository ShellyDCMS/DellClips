...
| File                               | Covers                                                                                                                                                                                                                                        |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cloudflare-video-service.test.ts` | `CloudflareVideoService` — upload URL, playback URL, delete, parseWebhook (empty/non-JSON/challenge/readyToStream/status ready/error/unknown)                                                                                                 |
| `demo-video-service.test.ts`       | `DemoVideoService` — fake upload, HLS playback URLs, no-op delete, parseWebhook (empty/non-JSON/challenge/unknown)                                                                                                                            |
...
| `gdrive-video-service.test.ts`     | `GDriveVideoService` — manual upload URL, gdrive playback URL, non-gdrive fallback, no-op delete, parseWebhook (empty/non-JSON/challenge/unknown)                                                                                             |
...
| `webhook.test.ts`           | `POST/HEAD /api/video/webhook`               | Delegates to videoService.parseWebhook, verification challenge echo, video_ready with/without assetId, video_error with/without assetId, unknown, DB errors, parseWebhook crash, body passthrough |
...
| `analytics.test.ts`         | `POST /api/analytics`                        | Anonymous tracking (null userId), authenticated tracking, eventType/videoId/metadata insert, missing eventType (400), DB errors (500)                                                             |
