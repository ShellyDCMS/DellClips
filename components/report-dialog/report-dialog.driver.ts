...
export class ReportDialogDriver extends BaseTestDriver<ReportDialogDriverProps> {
  ...
  when = {
    ...this._when,
    selectReason: (reasonCode: string) =>
      this.helper.when.click(`report-reason-${reasonCode}`),
  ...
  get = {
    ...
    reasonButton: (reasonCode: string) =>
      this.helper.get.elementByTestId(`report-reason-${reasonCode}`),
